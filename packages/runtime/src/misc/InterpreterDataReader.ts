...
export namespace InterpreterDataReader {
	...
	export async function parseFile(
		fileName: string,
	): Promise<InterpreterDataReader.InterpreterData> {
		let result: InterpreterDataReader.InterpreterData =
			new InterpreterDataReader.InterpreterData();
		...
		try {
			...
			for (let i: number = 0; i < elements.length; ++i) {
				...
				if (element.startsWith('[')) {
					...
					value = parseInt(
						element.substring(0, element.length - 1).trim(),
						10,
					);
				} else {
