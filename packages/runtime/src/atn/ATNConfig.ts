...
export class ATNConfig implements Equatable {
	...
	public static create(
		...
		alt: number,
		context: PredictionContext,
		@NotNull semanticContext: SemanticContext = SemanticContext.NONE,
		...
		if (semanticContext !== SemanticContext.NONE) {
			if (lexerActionExecutor != null) {
				...
				return new SemanticContextATNConfig(
					semanticContext,
					state,
					alt,
					context,
	...
	set outerContextDepth(outerContextDepth: number) {
		...
		this.altAndOuterContextDepth =
			(outerContextDepth << 24) |
			((this.altAndOuterContextDepth & ~0x7f000000) >>> 0);
	}
	...
	public transform(
		/*@NotNull*/ state: ATNState,
		checkNonGreedy: boolean,
		arg2?: SemanticContext | PredictionContext | LexerActionExecutor,
	): ATNConfig {
		if (arg2 == null) {
	...
	private transformImpl(
		...
		let passedThroughNonGreedy: boolean =
			checkNonGreedy && ATNConfig.checkNonGreedyDecision(this, state);
		if (semanticContext !== SemanticContext.NONE) {
	...
	private static checkNonGreedyDecision(
		...
		return (
			source.hasPassedThroughNonGreedyDecision ||
	...
	public appendContext(
		context: number | PredictionContext,
		contextCache: PredictionContextCache,
	): ATNConfig {
		if (typeof context === 'number') {
			let appendedContext: PredictionContext =
				this.context.appendSingleContext(context, contextCache);
			let result: ATNConfig = this.transform(
				this.state,
				false,
				appendedContext,
			);
			...
			let appendedContext: PredictionContext = this.context.appendContext(
				context,
				contextCache,
			);
			let result: ATNConfig = this.transform(
	...
	public contains(subconfig: ATNConfig): boolean {
		if (
			this.state.stateNumber !== subconfig.state.stateNumber ||
		...
		while (true) {
			...
			if (right.isEmpty) {
				...
				for (let i = 0; i < right.size; i++) {
					let index: number = left.findReturnState(
						right.getReturnState(i),
					);
					if (index < 0) {
	...
	get isPrecedenceFilterSuppressed(): boolean {
		return (
			(this.altAndOuterContextDepth & SUPPRESS_PRECEDENCE_FILTER) !== 0
		);
	}
	...
	public equals(o: any): boolean {
		...
		return (
			this.state.stateNumber === o.state.stateNumber &&
			this.alt === o.alt &&
			...
			this.isPrecedenceFilterSuppressed ===
				o.isPrecedenceFilterSuppressed &&
			this.hasPassedThroughNonGreedyDecision ===
				o.hasPassedThroughNonGreedyDecision &&
			ObjectEqualityComparator.INSTANCE.equals(
				this.lexerActionExecutor,
				o.lexerActionExecutor,
			)
		);
	}
	...
	public toDotString(): string {
		...
		let visited = new Array2DHashMap<PredictionContext, number>(
			PredictionContext.IdentityEqualityComparator.INSTANCE,
		);
...
class SemanticContextATNConfig extends ATNConfig {
	...
	constructor(
		semanticContext: SemanticContext,
		@NotNull state: ATNState,
		@NotNull altOrConfig: number | ATNConfig,
		context: PredictionContext,
	) {
		if (typeof altOrConfig === 'number') {
