...
export class ATNDeserializer {
	 ...
	 */
	private static readonly BASE_SERIALIZED_UUID: UUID = UUID.fromString(
	 ...
	 */
	private static readonly ADDED_LEXER_ACTIONS: UUID = UUID.fromString(
	 ...
	 */
	private static readonly ADDED_UNICODE_SMP: UUID = UUID.fromString(
	 ...
	 */
	protected static isFeatureSupported(
		feature: UUID,
		actualUuid: UUID,
	): boolean {
		let featureIndex: number = ATNDeserializer.SUPPORTED_UUIDS.findIndex(
			(e) => e.equals(feature),
		);
		if (featureIndex < 0) {
			return false;
		...
		return (
			ATNDeserializer.SUPPORTED_UUIDS.findIndex((e) =>
				e.equals(actualUuid),
			) >= featureIndex
		);
	}
	...
	public deserialize(@NotNull data: Uint16Array): ATN {
		...
		p += 8;
		if (
			ATNDeserializer.SUPPORTED_UUIDS.findIndex((e) => e.equals(uuid)) < 0
		) {
			let reason = `Could not deserialize ATN with UUID ${uuid} (expected ${ATNDeserializer.SERIALIZED_UUID} or a legacy UUID).`;
			throw new Error(reason);
		}

		let supportsLexerActions: boolean = ATNDeserializer.isFeatureSupported(
			ATNDeserializer.ADDED_LEXER_ACTIONS,
			uuid,
		);
		...
		for (let i = 0; i < nstates; i++) {
			...
			if (stype === ATNStateType.LOOP_END) {
				// special case
				let loopBackStateNumber: number = ATNDeserializer.toInt(
					data[p++],
				);
		...
		for (let i = 0; i < nrules; i++) {
			...
			if (atn.grammarType === ATNType.LEXER) {
				...
				if (
					...
					let actionIndexIgnored: number = ATNDeserializer.toInt(
						data[p++],
					);
		...
		p = this.deserializeSets(
			data,
			p,
			sets,
			ATNDeserializer.getUnicodeDeserializer(
				UnicodeDeserializingMode.UNICODE_BMP,
			),
		);
		...
		if (
			...
			p = this.deserializeSets(
				...
				sets,
				ATNDeserializer.getUnicodeDeserializer(
					UnicodeDeserializingMode.UNICODE_SMP,
				),
			);
		}
		...
		for (let i = 0; i < nedges; i++) {
			...
			let trans: Transition = this.edgeFactory(
				atn,
				ttype,
				src,
				trg,
		...
		let returnTransitionsSet = new Array2DHashSet<T>({
			hashCode: (o: T) =>
				o.stopState ^ o.returnState ^ o.outermostPrecedenceReturn,
		...
		for (let state of atn.states) {
			let returningToLeftFactored: boolean =
				state.ruleIndex >= 0 &&
			...
			for (let i = 0; i < state.numberOfTransitions; i++) {
				...
				let returningFromLeftFactored: boolean =
					atn.ruleToStartState[ruleTransition.target.ruleIndex]
						.leftFactored;
				...
				if (
					...
					if (ruleTransition.precedence === 0) {
						outermostPrecedenceReturn =
							ruleTransition.target.ruleIndex;
					}
				}
		...
		for (let returnTransition of returnTransitions) {
			let transition = new EpsilonTransition(
				atn.states[returnTransition.returnState],
				returnTransition.outermostPrecedenceReturn,
			);
			atn.ruleToStopState[returnTransition.stopState].addTransition(
				transition,
			);
		}
		...
		if (atn.grammarType === ATNType.LEXER) {
			if (supportsLexerActions) {
				atn.lexerActions = new Array<LexerAction>(
					ATNDeserializer.toInt(data[p++]),
				);
				for (let i = 0; i < atn.lexerActions.length; i++) {
					let actionType: LexerActionType = ATNDeserializer.toInt(
						data[p++],
					);
					...
					let lexerAction: LexerAction = this.lexerActionFactory(
						actionType,
						data1,
						data2,
					);
				...
				for (let state of atn.states) {
					for (let i = 0; i < state.numberOfTransitions; i++) {
						...
						let lexerAction: LexerCustomAction =
			...
			for (let i = 0; i < atn.ruleToStartState.length; i++) {
				...
				if (atn.ruleToStartState[i].isPrecedenceRule) {
					...
					for (let state of atn.states) {
						...
						let maybeLoopEndState: ATNState = state.transition(
						...
						if (
							...
							maybeLoopEndState.transition(0).target instanceof
								RuleStopState
						) {
							endState = state;
							break;
					...
					excludeTransition = (
						endState as StarLoopEntryState
					).loopBackState.transition(0);
				} else {
				...
				while (atn.ruleToStartState[i].numberOfTransitions > 0) {
					let transition: Transition = atn.ruleToStartState[
						i
					].removeTransition(
		...
		if (this.deserializationOptions.isOptimize) {
			while (true) {
				...
				optimizationCount +=
					ATNDeserializer.combineChainedEpsilons(atn);
				let preserveOrder: boolean = atn.grammarType === ATNType.LEXER;
				optimizationCount += ATNDeserializer.optimizeSets(
					atn,
					preserveOrder,
				);
	...
	protected markPrecedenceDecisions(@NotNull atn: ATN): void {
			...
			if (atn.ruleToStartState[state.ruleIndex].isPrecedenceRule) {
				...
				if (maybeLoopEndState instanceof LoopEndState) {
					if (
						...
						state.precedenceLoopbackStates = new BitSet(
							atn.states.length,
						);
					}
				}
		...
		for (let precedenceDecision of rulePrecedenceDecisions) {
			for (let transition of atn.ruleToStopState[
				precedenceDecision[0]
			].getTransitions()) {
				...
				precedenceDecision[1].precedenceLoopbackStates.set(
	...
	protected verifyATN(atn: ATN): void {
		...
		for (let state of atn.states) {
			...
			if (state instanceof StarLoopEntryState) {
				...
				this.checkCondition(
					starLoopEntryState.loopBackState !== undefined,
				);
				this.checkCondition(
					starLoopEntryState.numberOfTransitions === 2,
				);

				if (
					starLoopEntryState.transition(0).target instanceof
					StarBlockStartState
				) {
					this.checkCondition(
						starLoopEntryState.transition(1).target instanceof
							LoopEndState,
					);
			...
			if (state instanceof StarLoopbackState) {
				...
				this.checkCondition(
					state.transition(0).target instanceof StarLoopEntryState,
				);
			}
			...
			if (state instanceof DecisionState) {
				...
				this.checkCondition(
					decisionState.numberOfTransitions <= 1 ||
						decisionState.decision >= 0,
				);
			} else {
				this.checkCondition(
					state.numberOfTransitions <= 1 ||
						state instanceof RuleStopState,
				);
			}
		}
	}
	...
	private static inlineSetRules(atn: ATN): number {
		...
		let ruleToInlineTransition = new Array<Transition | undefined>(
		...
		for (let i = 0; i < atn.ruleToStartState.length; i++) {
			...
			while (
				middleState.onlyHasEpsilonTransitions &&
				middleState.numberOfOptimizedTransitions === 1 &&
				middleState.getOptimizedTransition(0).serializationType ===
			...
			let matchTransition: Transition =
				middleState.getOptimizedTransition(0);
			let matchTarget: ATNState = matchTransition.target;
			if (
				matchTransition.isEpsilon ||
				!matchTarget.onlyHasEpsilonTransitions ||
				matchTarget.numberOfOptimizedTransitions !== 1 ||
				!(
					matchTarget.getOptimizedTransition(0).target instanceof
					RuleStopState
				)
			) {
				continue;
		...
		for (let state of atn.states) {
			...
			for (let i = 0; i < state.numberOfOptimizedTransitions; i++) {
				...
				let effective: Transition | undefined =
					ruleToInlineTransition[ruleTransition.target.ruleIndex];
				...
				if (optimizedTransitions === undefined) {
					...
					for (let j = 0; j < i; j++) {
						optimizedTransitions.push(
							state.getOptimizedTransition(i),
						);
					}
				}
				...
				optimizedTransitions.push(
					new EpsilonTransition(intermediateState),
				);

				switch (effective.serializationType) {
					case TransitionType.ATOM:
						intermediateState.addTransition(
							new AtomTransition(
								target,
								(effective as AtomTransition)._label,
							),
						);
						break;
					...
					case TransitionType.RANGE:
						intermediateState.addTransition(
							new RangeTransition(
								target,
								(effective as RangeTransition).from,
					...
					case TransitionType.SET:
						intermediateState.addTransition(
							new SetTransition(
								target,
								(effective as SetTransition).label,
							),
						);
						break;
			...
			if (optimizedTransitions !== undefined) {
				if (state.isOptimized) {
					while (state.numberOfOptimizedTransitions > 0) {
						state.removeOptimizedTransition(
	...
	private static combineChainedEpsilons(atn: ATN): number {
		...
		for (let state of atn.states) {
			...
			nextTransition: for (
				let i = 0;
				i < state.numberOfOptimizedTransitions;
				i++
			) {
				let transition: Transition = state.getOptimizedTransition(i);
				let intermediate: ATNState = transition.target;
				if (
					transition.serializationType !== TransitionType.EPSILON ||
					(transition as EpsilonTransition)
				...
				for (
					let j = 0;
					j < intermediate.numberOfOptimizedTransitions;
					j++
				) {
					if (
						intermediate.getOptimizedTransition(j)
							.serializationType !== TransitionType.EPSILON ||
						(
							intermediate.getOptimizedTransition(
								j,
							) as EpsilonTransition
						).outermostPrecedenceReturn !== -1
					) {
						if (optimizedTransitions !== undefined) {
							optimizedTransitions.push(transition);
				...
				if (optimizedTransitions === undefined) {
					...
					for (let j = 0; j < i; j++) {
						optimizedTransitions.push(
							state.getOptimizedTransition(j),
						);
					}
				}
				...
				for (
					...
					let target: ATNState =
						intermediate.getOptimizedTransition(j).target;
	...
	private static optimizeSets(atn: ATN, preserveOrder: boolean): number {
		...
		for (let decision of decisions) {
			...
			for (let i = 0; i < decision.numberOfOptimizedTransitions; i++) {
				let epsTransition: Transition =
					decision.getOptimizedTransition(i);
				...
				let transition: Transition =
					epsTransition.target.getOptimizedTransition(0);
			...
			let blockEndState: ATNState = decision
				.getOptimizedTransition(setTransitions.minElement)
			...
			for (let interval of setTransitions.intervals) {
				for (let j = interval.a; j <= interval.b; j++) {
					let matchTransition: Transition = decision
						.getOptimizedTransition(j)
			...
			if (matchSet.intervals.length === 1) {
				if (matchSet.size === 1) {
					newTransition = new AtomTransition(
						blockEndState,
						matchSet.minElement,
					);
				} else {
					let matchInterval: Interval = matchSet.intervals[0];
					newTransition = new RangeTransition(
						blockEndState,
						matchInterval.a,
						matchInterval.b,
					);
			...
			removedPaths +=
				decision.numberOfOptimizedTransitions -
				optimizedTransitions.length;

			if (decision.isOptimized) {
				while (decision.numberOfOptimizedTransitions > 0) {
					decision.removeOptimizedTransition(
						decision.numberOfOptimizedTransitions - 1,
					);
				}
			}
	...
	private static identifyTailCalls(atn: ATN): void {
		for (let state of atn.states) {
			for (let i = 0; i < state.numberOfTransitions; i++) {
				...
				transition.optimizedTailCall = this.testTailCall(
					atn,
					transition,
					true,
				);
	...
	private static testTailCall(
		...
		while (true) {
			...
			let transitionCount =
				optimizedPath ?
					state.numberOfOptimizedTransitions
	...
	protected edgeFactory(
		...
		switch (type) {
			...
			case TransitionType.RULE:
				let rt: RuleTransition = new RuleTransition(
					atn.states[arg1] as RuleStartState,
					arg2,
					arg3,
					target,
				);
			...
			case TransitionType.PREDICATE:
				let pt: PredicateTransition = new PredicateTransition(
					target,
					arg1,
					arg2,
					arg3 !== 0,
			...
			case TransitionType.ACTION:
				let a: ActionTransition = new ActionTransition(
					target,
					arg1,
					arg2,
					arg3 !== 0,
	...
	protected stateFactory(type: ATNStateType, ruleIndex: number): ATNState {
		let s: ATNState;
		switch (type) {
			case ATNStateType.INVALID_TYPE:
