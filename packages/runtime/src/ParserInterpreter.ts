...
export class ParserInterpreter extends Parser {
	...
	constructor(
		...
		super(
			grammarFileName instanceof ParserInterpreter ?
				grammarFileName.inputStream
			:	input!,
		);
	...
	public parse(startRuleIndex: number): ParserRuleContext {
		let startRuleStartState: RuleStartState =
			this._atn.ruleToStartState[startRuleIndex];
		...
		if (startRuleStartState.isPrecedenceRule) {
			this.enterRecursionRule(
				this._rootContext,
				startRuleStartState.stateNumber,
				startRuleIndex,
				0,
			);
		} else {
				...
				case ATNStateType.RULE_STOP:
					...
					if (this._ctx.isEmpty) {
						if (startRuleStartState.isPrecedenceRule) {
							...
							let parentContext: [ParserRuleContext, number] =
								this._parentContextStack.pop()!;
	...
	protected visitState(p: ATNState): void {
		...
		switch (transition.serializationType) {
			case TransitionType.EPSILON:
				if (
					this.pushRecursionContextStates.get(p.stateNumber) &&
					...
					let localctx: InterpreterRuleContext =
						this.createInterpreterRuleContext(
							parentContext[0],
							parentContext[1],
							this._ctx.ruleIndex,
						);
			...
			case TransitionType.RULE:
				let ruleStartState: RuleStartState =
					transition.target as RuleStartState;
				let ruleIndex: number = ruleStartState.ruleIndex;
				let newctx: InterpreterRuleContext =
					this.createInterpreterRuleContext(
						this._ctx,
						p.stateNumber,
						ruleIndex,
			...
			case TransitionType.PREDICATE:
				let predicateTransition: PredicateTransition =
					transition as PredicateTransition;
				if (
					!this.sempred(
						this._ctx,
						predicateTransition.ruleIndex,
			...
			case TransitionType.ACTION:
				let actionTransition: ActionTransition =
					transition as ActionTransition;
				this.action(
					this._ctx,
			...
			case TransitionType.PRECEDENCE:
				if (
					!this.precpred(
						this._ctx,
						(transition as PrecedencePredicateTransition)
							.precedence,
					)
				) {
					let precedence = (
	...
	protected visitDecisionState(p: DecisionState): number {
		...
		if (
			...
			predictedAlt = this.interpreter.adaptivePredict(
				this._input,
				decision,
				this._ctx,
			);
	...
	protected visitRuleStopState(p: ATNState): void {
		...
		let ruleTransition: RuleTransition = this._atn.states[
			this.state
		].transition(0) as RuleTransition;
	...
	protected recover(e: RecognitionException): void {
		...
		if (this._input.index === i) {
			...
			if (e instanceof InputMismatchException) {
				...
				let errToken: Token = this.tokenFactory.create(
					sourcePair,
					expectedTokenType,
					tok.text,
				...
				this._ctx.addErrorNode(
					this.createErrorNode(this._ctx, errToken),
				);
			} else {
				// NoViableAlt
				let source = tok.tokenSource;
