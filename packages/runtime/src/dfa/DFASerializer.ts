...
export class DFASerializer {
	...
	constructor(
		/*@NotNull*/ dfa: DFA,
		/*@Nullable*/ parser: Recognizer<any, any> | undefined,
	);
	constructor(
		/*@NotNull*/ dfa: DFA,
		/*@NotNull*/ vocabulary: Vocabulary,
		/*@Nullable*/ ruleNames: string[] | undefined,
		/*@Nullable*/ atn: ATN | undefined,
	);
	constructor(
		dfa: DFA,
		vocabulary: Vocabulary | Recognizer<any, any> | undefined,
		ruleNames?: string[],
		atn?: ATN,
	) {
	...
	public toString(): string {
		...
		if (this.dfa.states) {
			let states: DFAState[] = new Array<DFAState>(
				...this.dfa.states.toArray(),
			);
			...
			for (let s of states) {
				...
				let contextEdgeKeys = [...contextEdges.keys()].sort(
					(a, b) => a - b,
				);
				for (let entry of edgeKeys) {
					let value = edges.get(entry);
					if (
						(value == null || value === ATNSimulator.ERROR) &&
						!s.isContextSymbol(entry)
					) {
						continue;
					}
					...
					buf +=
						this.getStateString(s) +
						'-' +
						this.getEdgeLabel(entry) +
						'->';
	...
	protected getContextLabel(i: number): string {
		...
		if (this.atn && i > 0 && i <= this.atn.states.length) {
			...
			if (
				this.ruleNames &&
				ruleIndex >= 0 &&
				ruleIndex < this.ruleNames.length
			) {
				return (
					'ctx:' + String(i) + '(' + this.ruleNames[ruleIndex] + ')'
				);
			}
		}
