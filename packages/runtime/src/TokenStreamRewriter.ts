...
export class TokenStreamRewriter {
	...
	public rollback(
		...
		programName: string = TokenStreamRewriter.DEFAULT_PROGRAM_NAME,
	): void {
		let is: RewriteOperation[] | undefined = this.programs.get(programName);
		if (is != null) {
			this.programs.set(
				programName,
				is.slice(TokenStreamRewriter.MIN_TOKEN_INDEX, instructionIndex),
			);
		}
	}
	...
	public insertBefore(
		...
		let rewrites: RewriteOperation[] = this.getProgram(programName);
		let op: RewriteOperation = new InsertBeforeOp(
			this.tokens,
			index,
			rewrites.length,
			text,
	...
	public replace(
		...
		let rewrites: RewriteOperation[] = this.getProgram(programName);
		let op: RewriteOperation = new ReplaceOp(
			this.tokens,
			from,
			to,
			rewrites.length,
	...
	protected getLastRewriteTokenIndex(
		...
		let I: number | undefined =
			this.lastRewriteTokenIndexes.get(programName);
		if (I == null) {
			return -1;
		}
	...
	public getText(
		intervalOrProgram?: Interval | string,
		programName: string = TokenStreamRewriter.DEFAULT_PROGRAM_NAME,
	): string {
		...
		let rewrites: RewriteOperation[] | undefined =
			this.programs.get(programName);
		...
		let indexToOp: Map<number, RewriteOperation> =
			this.reduceToSingleOperationPerIndex(rewrites);
	...
	protected reduceToSingleOperationPerIndex(
		...
		for (let i = 0; i < rewrites.length; i++) {
			...
			let inserts: InsertBeforeOp[] = this.getKindOfOps(
				rewrites,
				InsertBeforeOp,
				i,
			);
			...
			let prevReplaces: ReplaceOp[] = this.getKindOfOps(
				rewrites,
				ReplaceOp,
				i,
			);
		...
		for (let i = 0; i < rewrites.length; i++) {
			...
			let prevInserts: InsertBeforeOp[] = this.getKindOfOps(
				rewrites,
				InsertBeforeOp,
				i,
			);
		...
		let m: Map<number, RewriteOperation> = new Map<
			number,
			RewriteOperation
		>();
