...
function expectConsole(
	...
	if (
		output.length === expectedOutput.length - 1 &&
...
export interface ParserTestOptions<TParser extends Parser>
	extends LexerTestOptions {
	...
	expectConsole(options.expectedOutput, options.expectedErrors, () => {
		...
		if (options.showDFA) {
			process.stdout.write(
				lex.interpreter.getDFA(Lexer.DEFAULT_MODE).toLexerString(),
			);
		}
	});
}

export function parserTest<TParser extends Parser>(
	options: ParserTestOptions<TParser>,
) {
	const inputStream: CharStream = CharStreams.fromString(options.input);
	const lex = new options.lexer(inputStream);
	const tokens = new CommonTokenStream(lex);
