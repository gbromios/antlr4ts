...
import {
	ParseTreeMatcherX1Parser,
	RULE as ParseTreeMatcherX1Rule,
} from './gen/matcher/ParseTreeMatcherX1Parser';
import { ParseTreeMatcherX2Lexer } from './gen/matcher/ParseTreeMatcherX2Lexer';
import {
	ParseTreeMatcherX2Parser,
	RULE as ParseTreeMatcherX2Rule,
} from './gen/matcher/ParseTreeMatcherX2Parser';
import { ParseTreeMatcherX3Lexer } from './gen/matcher/ParseTreeMatcherX3Lexer';
import {
	ParseTreeMatcherX3Parser,
	RULE as ParseTreeMatcherX3Rule,
} from './gen/matcher/ParseTreeMatcherX3Parser';
import { ParseTreeMatcherX4Lexer } from './gen/matcher/ParseTreeMatcherX4Lexer';
import {
	ParseTreeMatcherX4Parser,
	RULE as ParseTreeMatcherX4Rule,
} from './gen/matcher/ParseTreeMatcherX4Parser';
import { ParseTreeMatcherX5Lexer } from './gen/matcher/ParseTreeMatcherX5Lexer';
import {
	ParseTreeMatcherX5Parser,
	RULE as ParseTreeMatcherX5Rule,
} from './gen/matcher/ParseTreeMatcherX5Parser';
import { ParseTreeMatcherX6Lexer } from './gen/matcher/ParseTreeMatcherX6Lexer';
import {
	ParseTreeMatcherX6Parser,
	RULE as ParseTreeMatcherX6Rule,
} from './gen/matcher/ParseTreeMatcherX6Parser';
import { ParseTreeMatcherX7Lexer } from './gen/matcher/ParseTreeMatcherX7Lexer';
import {
	ParseTreeMatcherX7Parser,
	RULE as ParseTreeMatcherX7Rule,
} from './gen/matcher/ParseTreeMatcherX7Parser';
import { ParseTreeMatcherX8Lexer } from './gen/matcher/ParseTreeMatcherX8Lexer';
import {
	ParseTreeMatcherX8Parser,
	RULE as ParseTreeMatcherX8Rule,
} from './gen/matcher/ParseTreeMatcherX8Parser';
...
export class TestParseTreeMatcher {
	@Test public testChunking(): void {
		let m: ParseTreePatternMatcher = this.getPatternMatcher(
			ParseTreeMatcherX1Lexer,
			ParseTreeMatcherX1Parser,
		);
		assert.strictEqual(
			m.split('<ID> = <expr> ;').toString(),
			"ID,' = ',expr,' ;'",
		);
		assert.strictEqual(
			m.split(' <ID> = <expr>').toString(),
		...
		assert.strictEqual(
			m.split('foo \\<x\\> bar <tag>').toString(),
			"'foo <x> bar ',tag",
		);
	}
	...
	@Test public testInvertedTags(): void {
		...
		assert.throws(
			() => m.split('>expr<'),
			/^Error: tag delimiters out of order in pattern: >expr<$/,
		);
	}
	...
	@Test public testUnclosedTag(): void {
		...
		assert.throws(
			() => m.split('<expr hi mom'),
			/^Error: unterminated tag in pattern: <expr hi mom$/,
		);
	}
	...
	@Test public testExtraClose(): void {
		...
		assert.throws(
			() => m.split('<expr> >'),
			/^Error: missing start tag in pattern: <expr> >$/,
		);
	}
	...
	@Test public testTokenizingPattern(): void {
		...
		let expected: string =
			"ID:3,[@-1,1:1='=',<1>,1:1],expr:7,[@-1,1:1=';',<2>,1:1]";
	...
	@Test public testCompilingPattern(): void {
		...
		let t: ParseTreePattern = m.compile(
			'<ID> = <expr> ;',
			ParseTreeMatcherX1Rule.s,
		);
	...
	@Test public testCompilingPatternConsumesAllTokens(): void {
		...
		assert.throws(
			() => m.compile('<ID> = <expr> ; extra', ParseTreeMatcherX1Rule.s),
	...
	@Test public testPatternMatchesStartRule(): void {
		...
		assert.throws(
			() => m.compile('<ID> ;', ParseTreeMatcherX1Rule.s),
	...
	@Test public testPatternMatchesStartRule2(): void {
		let m: ParseTreePatternMatcher = this.getPatternMatcher(
			ParseTreeMatcherX3Lexer,
			ParseTreeMatcherX3Parser,
		);
		assert.throws(
			() => m.compile('<ID> <ID> ;', ParseTreeMatcherX3Rule.s),
	...
	@Test public testHiddenTokensNotSeenByTreePatternParser(): void {
		let m: ParseTreePatternMatcher = this.getPatternMatcher(
			ParseTreeMatcherX2Lexer,
			ParseTreeMatcherX2Parser,
		);

		let t: ParseTreePattern = m.compile(
			'<ID> = <expr> ;',
			ParseTreeMatcherX3Rule.s,
		);
		let results: string = t.patternTree.toStringTree(m.parser);
		let expected: string = '(s <ID> = (expr <expr>) ;)';
		assert.strictEqual(results, expected);
	...
	@Test public testCompilingMultipleTokens(): void {
		let m: ParseTreePatternMatcher = this.getPatternMatcher(
			ParseTreeMatcherX4Lexer,
			ParseTreeMatcherX4Parser,
		);

		let t: ParseTreePattern = m.compile(
			'<ID> = <ID> ;',
			ParseTreeMatcherX4Rule.s,
		);
		let results: string = t.patternTree.toStringTree(m.parser);
	...
	@Test public async testIDNodeMatches(): Promise<void> {
		...
		await this.checkPatternMatch(
			(parser) => parser.s(),
			ParseTreeMatcherX5Rule.s,
			input,
			pattern,
			ParseTreeMatcherX5Lexer,
			ParseTreeMatcherX5Parser,
		);
	}
	...
	@Test public async testIDNodeWithLabelMatches(): Promise<void> {
		...
		let m: ParseTreeMatch = await this.checkPatternMatch(
			(parser) => parser.s(),
			ParseTreeMatcherX5Rule.s,
			input,
			pattern,
	...
	@Test public async testLabelGetsLastIDNode(): Promise<void> {
		...
		let m: ParseTreeMatch = await this.checkPatternMatch(
			(parser) => parser.s(),
			ParseTreeMatcherX6Rule.s,
			input,
			pattern,
			ParseTreeMatcherX6Lexer,
			ParseTreeMatcherX6Parser,
		);
		assert.strictEqual([...m.labels.keys()].toString(), 'ID,id');
	...
	@Test public async testIDNodeWithMultipleLabelMatches(): Promise<void> {
		...
		let m: ParseTreeMatch = await this.checkPatternMatch(
			(parser) => parser.s(),
			ParseTreeMatcherX7Rule.s,
			input,
			pattern,
			ParseTreeMatcherX7Lexer,
			ParseTreeMatcherX7Parser,
		);
	...
	@Test public async testAssign(): Promise<void> {
		...
		await this.checkPatternMatch(
			(parser) => parser.s(),
			ParseTreeMatcherX8Rule.s,
			input,
			pattern,
			ParseTreeMatcherX8Lexer,
			ParseTreeMatcherX8Parser,
		);
	}
	...
	private async checkPatternMatch<TParser extends Parser>(
	...
	): Promise<ParseTreeMatch> {
		let result: ParseTree = this.execParser(
			startRule,
			input,
			lexerCtor,
			parserCtor,
		...
		let p: ParseTreePattern = await this.getPattern(
			lexerCtor,
			parserCtor,
			pattern,
			startRuleIndex,
	...
	private async getPattern(
		lexerCtor: { new (stream: CharStream): Lexer },
		parserCtor: { new (stream: TokenStream): Parser },
		pattern: string,
		ruleIndex: number,
