...
export class TestXPath {
	@Test public testValidPaths(): void {
		...
		for (let i = 0; i < xpath.length; i++) {
			let nodes: string[] = this.getNodeStrings(
				SAMPLE_PROGRAM,
				xpath[i],
				(parser) => parser.prog(),
				TestXPathLexer,
	...
	@Test public testWeirdChar(): void {
		let path: string = '&';
		let expected: RegExp =
			/^RangeError: Invalid tokens or characters at index 0 in path '&' -- $/;
	...
	@Test public testWeirdChar2(): void {
		let path: string = '//w&e/';
		let expected: RegExp =
			/^RangeError: Invalid tokens or characters at index 3 in path '\/\/w&e\/' -- $/;
	...
	@Test public testBadTokenName(): void {
		let path: string = '//Ick';
		let expected: RegExp =
			/^Error: Ick at index 2 isn't a valid token name$/;
	...
	@Test public testBadRuleName(): void {
		...
		let expected: RegExp =
			/^Error: ick at index 6 isn't a valid rule name$/;
