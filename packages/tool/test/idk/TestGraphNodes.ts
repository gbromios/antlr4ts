...
export class TestGraphNodes {
	...
	@Test public test_properties(): void {
		...
		assert(
			!PredictionContext.EMPTY_FULL.equals(PredictionContext.EMPTY_LOCAL),
			'by value equal',
		);
	}

	@Test public test_$_$(): void {
		let r: PredictionContext = this.contextCache.join(
			PredictionContext.EMPTY_LOCAL,
			PredictionContext.EMPTY_LOCAL,
		);
	...
	@Test public test_Aaxc_Aayd(): void {
	...
	@Test public test_Aaubv_Acwdx(): void {
	...
	@Test public test_Aaubv_Abvdx(): void {
	...
	@Test public test_Aaubv_Abwdx(): void {
	...
	@Test public test_Aaubv_Abvdu(): void {
	...
	@Test public test_Aaubu_Acudu(): void {
	...
	for (let current = workList.pop(); !!current; current = workList.pop()) {
		...
		if (current.isEmpty) {
			...
			for (let i = 0; i < current.size; i++) {
				...
				nodes += '<p' + i + '>';
				if (
					current.getReturnState(i) ===
					PredictionContext.EMPTY_FULL_STATE_KEY
				) {
					nodes += '$';
				} else if (
					current.getReturnState(i) ===
					PredictionContext.EMPTY_LOCAL_STATE_KEY
				) {
					nodes += '*';
				}
			}
