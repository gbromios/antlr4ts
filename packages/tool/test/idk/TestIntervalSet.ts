...
export class TestIntervalSet {
	...
	@Test public testMin(): void {
		...
		assertEquals(
			Token.EOF,
			IntervalSet.COMPLETE_CHAR_SET.or(IntervalSet.of(Token.EOF))
				.minElement,
		);
	}
	...
	@Test public testRangeAndIsolatedElement(): void {
		let s: IntervalSet = IntervalSet.of(
			'a'.charCodeAt(0),
			'z'.charCodeAt(0),
		);
	...
	@Test public testEmptyIntersection(): void {
		...
		let s2: IntervalSet = IntervalSet.of(
			'0'.charCodeAt(0),
			'9'.charCodeAt(0),
		);
	...
	@Test public testComplement3(): void {
		...
		let result: string = s
			.complementRange(1, Lexer.MAX_CHAR_VALUE)
			.toString();
		assertEquals(expecting, result);
	}
