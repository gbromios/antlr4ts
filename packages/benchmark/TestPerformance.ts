...
export class TestPerformance {
	 ...
	 * grammar.
	 */
	private static readonly DEBUG_TEMPLATES_WAIT: boolean =
		TestPerformance.DEBUG_TEMPLATES;
	/**
	...
	public static readonly REPORT_CONTEXT_SENSITIVITY: boolean =
		TestPerformance.REPORT_FULL_CONTEXT;
	...
	private static readonly sharedLexers: Array<Lexer | undefined> =
		new Array<Lexer>(TestPerformance.NUMBER_OF_THREADS);
	private static readonly sharedLexerATNs: Array<ATN | undefined> =
		new Array<ATN>(TestPerformance.NUMBER_OF_THREADS);

	private static readonly sharedParsers: Array<AnyJavaParser | undefined> =
		new Array<AnyJavaParser>(TestPerformance.NUMBER_OF_THREADS);
	private static readonly sharedParserATNs: Array<ATN | undefined> =
		new Array<ATN>(TestPerformance.NUMBER_OF_THREADS);

	private static readonly sharedListeners: Array<
		ParseTreeListener | undefined
	> = new Array<ParseTreeListener>(TestPerformance.NUMBER_OF_THREADS);

	private static readonly totalTransitionsPerFile: Uint32Array[] =
		new Array<Uint32Array>(TestPerformance.PASSES);
	private static readonly computedTransitionsPerFile: Uint32Array[] =
		new Array<Uint32Array>(TestPerformance.PASSES);

	private static decisionInvocationsPerFile: Uint32Array[][] = new Array<
		Uint32Array[]
	>(TestPerformance.PASSES);
	private static fullContextFallbackPerFile: Uint32Array[][] = new Array<
		Uint32Array[]
	>(TestPerformance.PASSES);
	private static nonSllPerFile: Uint32Array[][] = new Array<Uint32Array[]>(
		TestPerformance.PASSES,
	);
	private static totalTransitionsPerDecisionPerFile: Uint32Array[][] =
		new Array<Uint32Array[]>(TestPerformance.PASSES);
	private static computedTransitionsPerDecisionPerFile: Uint32Array[][] =
		new Array<Uint32Array[]>(TestPerformance.PASSES);
	private static fullContextTransitionsPerDecisionPerFile: Uint32Array[][] =
		new Array<Uint32Array[]>(TestPerformance.PASSES);

	private static readonly timePerFile: Float64Array[] =
		new Array<Float64Array>(TestPerformance.PASSES);
	private static readonly tokensPerFile: Int32Array[] = new Array<Int32Array>(
		TestPerformance.PASSES,
	);

	private static readonly tokenCount: Int32Array = new Int32Array(
		TestPerformance.PASSES,
	);
	...
	public compileJdk(): void {
		...
		assertTrue(
			jdkSourceRoot != null && jdkSourceRoot.length > 0,
			'The JDK_SOURCE_ROOT environment variable must be set for performance testing.',
		);
		jdkSourceRoot = jdkSourceRoot as string;

		let lexerCtor: { new (input: CharStream): GeneratedJavaLexer } =
			TestPerformance.USE_LR_GRAMMAR ? JavaLRLexer : JavaLexer;
		let parserCtor: { new (input: TokenStream): GeneratedJavaParser } =
			TestPerformance.USE_LR_GRAMMAR ? JavaLRParser : JavaParser;
		if (TestPerformance.FORCE_ATN) {
			lexerCtor =
				TestPerformance.USE_LR_GRAMMAR ? JavaLRLexerAtn : JavaLexerAtn;
			parserCtor =
				TestPerformance.USE_LR_GRAMMAR ? JavaLRParserAtn : (
					JavaParserAtn
				);
		} else {
			lexerCtor =
		...
		let listenerName: string =
			TestPerformance.USE_LR_GRAMMAR ? 'JavaLRBaseListener' : (
				'JavaBaseListener'
			);
		...
		if (TestPerformance.TOP_PACKAGE.length > 0) {
			jdkSourceRoot =
				jdkSourceRoot +
				'/' +
				TestPerformance.TOP_PACKAGE.replace(/\./g, '/');
		}

		let directory: string = jdkSourceRoot;
		assertTrue(fs.lstatSync(directory).isDirectory());

		let filesFilter: FilenameFilter = FilenameFilters.extension(
			'.java',
			false,
		);
		let directoriesFilter: FilenameFilter = FilenameFilters.ALL_FILES;
		let sources: InputDescriptor[] = this.loadSources(
			directory,
			filesFilter,
			directoriesFilter,
			TestPerformance.RECURSIVE,
		);

		for (let i = 0; i < TestPerformance.PASSES; i++) {
			if (TestPerformance.COMPUTE_TRANSITION_STATS) {
				TestPerformance.totalTransitionsPerFile[i] = new Uint32Array(
					Math.min(
						sources.length,
						TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					),
				);
				TestPerformance.computedTransitionsPerFile[i] = new Uint32Array(
					Math.min(
						sources.length,
						TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					),
				...
				if (TestPerformance.DETAILED_DFA_STATE_STATS) {
					TestPerformance.decisionInvocationsPerFile[i] =
						new Array<Uint32Array>(
							Math.min(
								sources.length,
								TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
							),
						);
					TestPerformance.fullContextFallbackPerFile[i] =
						new Array<Uint32Array>(
							Math.min(
								sources.length,
								TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					...
					TestPerformance.nonSllPerFile[i] = new Array<Uint32Array>(
						Math.min(
							sources.length,
							TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
						),
					);
					TestPerformance.totalTransitionsPerDecisionPerFile[i] =
						new Array<Uint32Array>(
							Math.min(
								sources.length,
								TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					...
					TestPerformance.computedTransitionsPerDecisionPerFile[i] =
						new Array<Uint32Array>(
							Math.min(
								sources.length,
								TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					...
					TestPerformance.fullContextTransitionsPerDecisionPerFile[
						i
					] = new Array<Uint32Array>(
						Math.min(
							sources.length,
			...
			if (TestPerformance.COMPUTE_TIMING_STATS) {
				TestPerformance.timePerFile[i] = new Float64Array(
					Math.min(
						sources.length,
						TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					),
				);
				TestPerformance.tokensPerFile[i] = new Int32Array(
					Math.min(
						sources.length,
						TestPerformance.MAX_FILES_PER_PARSE_ITERATION,
					),
		...
		process.stdout.write(
			TestPerformance.getOptionsDescription(TestPerformance.TOP_PACKAGE),
		);
		...
		try {
			this.parse1(
				...
				factory,
				sources,
				TestPerformance.SHUFFLE_FILES_AT_START,
			);
		} catch (ex) {
		...
		for (let i = 0; i < TestPerformance.PASSES - 1; i++) {
			...
			if (TestPerformance.CLEAR_DFA) {
				...
				if (
					TestPerformance.sharedLexers.length > 0 &&
					TestPerformance.sharedLexers[index] != null
				) {
				...
				if (
					TestPerformance.sharedParsers.length > 0 &&
					TestPerformance.sharedParsers[index] != null
				) {
			...
			try {
				this.parse2(
					...
					factory,
					sources,
					TestPerformance.SHUFFLE_FILES_AFTER_ITERATIONS,
				);
			} catch (ex) {
				// Logger.getLogger(TestPerformance.class.getName()).log(Level.SEVERE, null, ex);
				console.error(ex);
		...
		if (
			...
			TestPerformance.SHOW_TRANSITION_STATS_PER_FILE
		) {
	...
	private computeTransitionStatistics(): void {
		if (TestPerformance.TRANSITION_RUNNING_AVERAGE) {
			for (let i = 0; i < TestPerformance.PASSES; i++) {
				let data: Uint32Array =
					TestPerformance.computedTransitionsPerFile[i];
		...
		let sumNum: Uint32Array = new Uint32Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let sumDen: Uint32Array = new Uint32Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let sumNormalized: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		for (let i = 0; i < TestPerformance.PASSES; i++) {
			let num: Uint32Array =
		...
		let weightedAverage: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let average: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		...
		let low95: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let high95: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let low67: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let high67: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		let stddev: Float64Array = new Float64Array(
			TestPerformance.totalTransitionsPerFile[0].length,
		);
		for (let i = 0; i < stddev.length; i++) {
			...
			for (let j = 0; j < TestPerformance.PASSES; j++) {
				let totalTransitions: number =
					TestPerformance.totalTransitionsPerFile[j][i];
				if (totalTransitions > 0) {
					points[j] =
						TestPerformance.computedTransitionsPerFile[j][i] /
						TestPerformance.totalTransitionsPerFile[j][i];
				} else {
					points[j] = 0;
				}
			...
			let averageValue: number =
				TestPerformance.TRANSITION_WEIGHTED_AVERAGE ?
					weightedAverage[i]
				:	average[i];
			let value: number = 0;
			for (let j = 0; j < TestPerformance.PASSES; j++) {
			...
			let ignoreCount95: number =
				Math.round((TestPerformance.PASSES * (1 - 0.95)) / 2.0) | 0;
			let ignoreCount67: number =
				Math.round((TestPerformance.PASSES * (1 - 0.667)) / 2.0) | 0;
		...
		console.log(
			'File\tAverage\tStd. Dev.\t95%% Low\t95%% High\t66.7%% Low\t66.7%% High',
		);
		for (let i = 0; i < stddev.length; i++) {
			let averageValue: number =
				TestPerformance.TRANSITION_WEIGHTED_AVERAGE ?
				...
				:	average[i];
			console.log(
				`${i + 1}\t${averageValue}\t${stddev[i]}\t${
					averageValue - low95[i]
				}\t${high95[i] - averageValue}\t${averageValue - low67[i]}\t${
	...
	private computeTimingStatistics(): void {
		...
		for (let i = 0; i < stddev.length; i++) {
			...
			for (let j = 0; j < TestPerformance.PASSES; j++) {
				points[j] =
					TestPerformance.timePerFile[j][i] /
					TestPerformance.tokensPerFile[j][i];
			}
	...
	public static getOptionsDescription(topPackage: string): string {
		...
		builder +=
			', Grammar=' + (TestPerformance.USE_LR_GRAMMAR ? 'LR' : 'Standard');
		builder += ', ForceAtn=' + TestPerformance.FORCE_ATN;
		builder +=
			', Lexer:' + (TestPerformance.ENABLE_LEXER_DFA ? 'DFA' : 'ATN');
		builder +=
			', Parser:' + (TestPerformance.ENABLE_PARSER_DFA ? 'DFA' : 'ATN');
		...
		builder +=
			', Strategy=' +
			(TestPerformance.BAIL_ON_ERROR ?
				BailErrorStrategy.name
			:	DefaultErrorStrategy.name);
		...
		builder +=
			'Lexer=' +
			(TestPerformance.REUSE_LEXER ? 'setInputStream' : 'newInstance');
		builder +=
			', Parser=' +
			(TestPerformance.REUSE_PARSER ? 'setInputStream' : 'newInstance');
		builder +=
			', AfterPass=' +
			(TestPerformance.CLEAR_DFA ? 'newInstance' : 'setInputStream');

		builder += '\n';

		builder +=
			'UniqueClosure=' +
			(TestPerformance.OPTIMIZE_UNIQUE_CLOSURE ? 'optimize' : 'complete');
	...
	protected loadSources(
		directory: string,
		filesFilter: FilenameFilter,
		directoriesFilter: FilenameFilter,
		recursive: boolean,
	): InputDescriptor[];
	protected loadSources(
		directory: string,
	...
	protected loadSources(
		...
		if (recursive) {
			let children: string[] = listFilesSync(
				directory,
				directoriesFilter,
			);
	...
	protected parseSources(
		...
		for (let future of results) {
			...
			if (TestPerformance.COMPUTE_TRANSITION_STATS) {
				TestPerformance.totalTransitionsPerFile[currentPass][
					currentIndex
				] = TestPerformance.sum(fileResult.parserTotalTransitions);
				TestPerformance.computedTransitionsPerFile[currentPass][
					currentIndex
				] = TestPerformance.sum(fileResult.parserComputedTransitions);

				if (TestPerformance.DETAILED_DFA_STATE_STATS) {
					TestPerformance.decisionInvocationsPerFile[currentPass][
						currentIndex
					] = fileResult.decisionInvocations;
					TestPerformance.fullContextFallbackPerFile[currentPass][
						currentIndex
					] = fileResult.fullContextFallback;
					TestPerformance.nonSllPerFile[currentPass][currentIndex] =
						fileResult.nonSll;
					TestPerformance.totalTransitionsPerDecisionPerFile[
						currentPass
					][currentIndex] = fileResult.parserTotalTransitions;
					TestPerformance.computedTransitionsPerDecisionPerFile[
						currentPass
					][currentIndex] = fileResult.parserComputedTransitions;
					TestPerformance.fullContextTransitionsPerDecisionPerFile[
						currentPass
					][currentIndex] = fileResult.parserFullContextTransitions;
				}
			}

			if (TestPerformance.COMPUTE_TIMING_STATS) {
				TestPerformance.timePerFile[currentPass][currentIndex] =
					fileResult.elapsedTime.totalMilliseconds;
				TestPerformance.tokensPerFile[currentPass][currentIndex] =
					fileResult.tokenCount;
			}
		...
		console.log(
			`${
				currentPass + 1
			}. Total parse time for ${inputCount} files (${Math.round(
				inputSize / 1024,
			)} KiB, ${TestPerformance.tokenCount[currentPass]} tokens${
				TestPerformance.COMPUTE_CHECKSUM ?
					`, checksum 0x${(checksum.getValue() >>> 0).toString(16)}`
				:	''
			}): ${Math.round(startTime.elapsedMillis())}ms`,
		);
		...
		if (TestPerformance.sharedLexers.length > 0) {
			...
			if (TestPerformance.SHOW_DFA_STATE_STATS) {
				...
				let uniqueConfigs: Array2DHashSet<ATNConfig> =
					new Array2DHashSet<ATNConfig>(
				...
				console.log(
					`There are ${states} lexer DFAState instances, ${configs} configs (${uniqueConfigs.size} unique), ${lexerInterpreter.atn.contextCacheSize} prediction contexts.`,
				);
				...
				if (TestPerformance.DETAILED_DFA_STATE_STATS) {
					...
					for (let i = 0; i < modeToDFA.length; i++) {
						...
						console.log(
							`\t${dfa.decision}\t${dfa.states.size}\t${modeConfigs}\t${modeName}`,
						);
					}
				}
			}
		...
		if (
			...
			if (TestPerformance.SHOW_DFA_STATE_STATS) {
				...
				console.log(
					`There are ${states} parser DFAState instances, ${configs} configs (${uniqueConfigs.size} unique), ${interpreter.atn.contextCacheSize} prediction contexts.`,
				);

				if (TestPerformance.DETAILED_DFA_STATE_STATS) {
					if (TestPerformance.COMPUTE_TRANSITION_STATS) {
						console.log(
							'\tDecision\tStates\tConfigs\tPredict (ALL)\tPredict (LL)\tNon-SLL\tTransitions\tTransitions (ATN)\tTransitions (LL)\tLA (SLL)\tLA (LL)\tRule',
						);
					} else {
					...
					for (let i = 0; i < decisionToDFA.length; i++) {
						...
						let ruleName: string =
							parser.ruleNames[
								parser.atn.decisionToState[dfa.decision]
									.ruleIndex
							];
						...
						if (TestPerformance.COMPUTE_TRANSITION_STATS) {
							for (let data of TestPerformance
								.decisionInvocationsPerFile[currentPass]) {
								calls += data[i];
							}

							for (let data of TestPerformance
								.fullContextFallbackPerFile[currentPass]) {
								fullContextCalls += data[i];
							}

							for (let data of TestPerformance.nonSllPerFile[
								currentPass
							]) {
								nonSllCalls += data[i];
							}
							...
							for (let data of TestPerformance
								.totalTransitionsPerDecisionPerFile[
								currentPass
							]) {
								transitions += data[i];
							}
							...
							for (let data of TestPerformance
								.computedTransitionsPerDecisionPerFile[
								currentPass
							]) {
							...
							if (calls > 0) {
								lookahead =
									(transitions - fullContextTransitions) /
									calls;
							}

							if (fullContextCalls > 0) {
								fullContextLookahead =
									fullContextTransitions / fullContextCalls;
							}
				...
				if (TestPerformance.SHOW_CONFIG_STATS) {
					for (let state of dfa.states) {
						if (state.configs.size >= contextsInDFAState.length) {
							let contextsInDFAState2 = new Int32Array(
								state.configs.size + 1,
							);
			...
			if (TestPerformance.SHOW_CONFIG_STATS && currentPass === 0) {
				console.log(
					`  DFA accept states: ${
						localDfaCount + globalDfaCount
					} total, ${localDfaCount} with only local context, ${globalDfaCount} with a global context`,
				);
				console.log(
					`  Config stats: ${
						localConfigCount + globalConfigCount
					} total, ${localConfigCount} local, ${globalConfigCount} global`,
				);
				if (TestPerformance.SHOW_DFA_STATE_STATS) {
					for (let i = 0; i < contextsInDFAState.length; i++) {
						if (contextsInDFAState[i] !== 0) {
							console.log(
								`  ${i} configs = ${contextsInDFAState[i]}`,
							);
						}
					}
				}
		...
		if (TestPerformance.COMPUTE_TIMING_STATS) {
			...
			for (
				let i = 0;
				i < TestPerformance.timePerFile[currentPass].length;
				i++
			) {
				console.log(
					`${i + 1}\t${
						TestPerformance.tokensPerFile[currentPass][i]
					}\t${TestPerformance.timePerFile[currentPass][i]}`,
				);
			}
		}
	}
	...
	public static updateChecksum(
		checksum: MurmurHashChecksum,
		value: number | Token | undefined,
	): void {
	...
	protected getParserFactory(
		lexerCtor: { new (input: CharStream): GeneratedJavaLexer },
		parserCtor: { new (input: TokenStream): GeneratedJavaParser },
		listenerCtor: { new (): ParseTreeListener },
		entryPointName: string,
		entryPoint: (parser: GeneratedJavaParser) => ParserRuleContext,
	): ParserFactory {
		// try {
		...
		let parserInstance = new parserCtor(
			new CommonTokenStream(lexerInstance),
		);
		...
		if (!TestPerformance.REUSE_LEXER_DFA) {
			...
			for (let i = 0; i < TestPerformance.NUMBER_OF_THREADS; i++) {
				TestPerformance.sharedLexerATNs[i] =
					new ATNDeserializer().deserialize(
						Utils.toCharArray(lexerSerializedATN),
					);
			}
		}
		...
		if (TestPerformance.RUN_PARSER && !TestPerformance.REUSE_PARSER_DFA) {
			...
			for (let i = 0; i < TestPerformance.NUMBER_OF_THREADS; i++) {
				TestPerformance.sharedParserATNs[i] =
					new ATNDeserializer().deserialize(
						Utils.toCharArray(parserSerializedATN),
					);
			}
		}
		...
		return {
			...
			parseFile(
				...
				let startTime: Stopwatch = Stopwatch.startNew();
				assert(
					thread >= 0 && thread < TestPerformance.NUMBER_OF_THREADS,
				);

				try {
					let listener: ParseTreeListener | undefined =
						TestPerformance.sharedListeners[thread];
					if (listener == null) {
					...
					let lexer: Lexer | undefined =
						TestPerformance.sharedLexers[thread];
					if (TestPerformance.REUSE_LEXER && lexer != null) {
						...
						let atn: ATN = (
							(
								TestPerformance.FILE_GRANULARITY ||
								previousLexer == null
							) ?
								lexer
							:	previousLexer).atn;
						if (
							!TestPerformance.REUSE_LEXER_DFA ||
							(!TestPerformance.FILE_GRANULARITY &&
								previousLexer == null)
						) {
						...
						if (!TestPerformance.ENABLE_LEXER_DFA) {
							lexer.interpreter = new NonCachingLexerATNSimulator(
								atn,
								lexer,
							);
						} else if (
							...
							lexer.interpreter = new StatisticsLexerATNSimulator(
								atn,
								lexer,
							);
						}
					...
					lexer.addErrorListener(
						DescriptiveLexerErrorListener.INSTANCE,
					);

					lexer.interpreter.optimize_tail_calls =
						TestPerformance.OPTIMIZE_TAIL_CALLS;
					if (
						TestPerformance.ENABLE_LEXER_DFA &&
						!TestPerformance.REUSE_LEXER_DFA
					) {
						lexer.interpreter.atn.clearDFA();
					}

					let tokens: CommonTokenStream = new CommonTokenStream(
						lexer,
					);
					tokens.fill();
					...
					let parser: AnyJavaParser | undefined =
						TestPerformance.sharedParsers[thread];
					if (TestPerformance.REUSE_PARSER && parser != null) {
						...
						if (TestPerformance.USE_PARSER_INTERPRETER) {
							let referenceParser: Parser = new parserCtor(
								tokens,
							);
							parser = new ParserInterpreter(
								referenceParser.grammarFileName,
								referenceParser.vocabulary,
								referenceParser.ruleNames,
								referenceParser.atn,
								tokens,
							);
						} else {
						...
						let atn: ATN = (
							...
							:	previousParser).atn;
						if (
							!TestPerformance.REUSE_PARSER_DFA ||
							(!TestPerformance.FILE_GRANULARITY &&
								previousParser == null)
						) {
						...
						if (!TestPerformance.ENABLE_PARSER_DFA) {
							parser.interpreter =
								new NonCachingParserATNSimulator(atn, parser);
						} else if (
							!TestPerformance.REUSE_PARSER_DFA ||
							TestPerformance.COMPUTE_TRANSITION_STATS
						) {
							parser.interpreter =
								new StatisticsParserATNSimulator(atn, parser);
						}
					...
					if (!TestPerformance.TWO_STAGE_PARSING) {
						...
						parser.addErrorListener(
							new SummarizingDiagnosticErrorListener(),
						);
					}

					if (
						TestPerformance.ENABLE_PARSER_DFA &&
						!TestPerformance.REUSE_PARSER_DFA
					) {
						parser.interpreter.atn.clearDFA();
					}

					parser.interpreter.setPredictionMode(
						TestPerformance.TWO_STAGE_PARSING ?
							PredictionMode.SLL
						:	TestPerformance.PREDICTION_MODE,
					);
					parser.interpreter.force_global_context =
						TestPerformance.FORCE_GLOBAL_CONTEXT &&
						!TestPerformance.TWO_STAGE_PARSING;
					parser.interpreter.always_try_local_context =
						TestPerformance.TRY_LOCAL_CONTEXT_FIRST ||
						TestPerformance.TWO_STAGE_PARSING;
					parser.interpreter.enable_global_context_dfa =
						TestPerformance.ENABLE_PARSER_FULL_CONTEXT_DFA;
					parser.interpreter.optimize_ll1 =
						TestPerformance.OPTIMIZE_LL1;
					parser.interpreter.optimize_unique_closure =
						TestPerformance.OPTIMIZE_UNIQUE_CLOSURE;
					parser.interpreter.optimize_tail_calls =
						TestPerformance.OPTIMIZE_TAIL_CALLS;
					parser.interpreter.tail_call_preserves_sll =
						TestPerformance.TAIL_CALL_PRESERVES_SLL;
					parser.interpreter.treat_sllk1_conflict_as_ambiguity =
						TestPerformance.TREAT_SLLK1_CONFLICT_AS_AMBIGUITY;
					parser.buildParseTree = TestPerformance.BUILD_PARSE_TREES;
					if (
						!TestPerformance.BUILD_PARSE_TREES &&
						TestPerformance.BLANK_LISTENER
					) {
					...
					try {
						if (
							...
							parser.addParseListener(
								new ChecksumParseTreeListener(checksum),
							);
						}

						if (parser instanceof ParserInterpreter) {
							parseResult = parser.parse(
								parser.ruleNames.indexOf(entryPointName),
							);
						} else {
						...
						if (TestPerformance.REPORT_SECOND_STAGE_RETRY) {
							console.error(
								sourceName +
									'Forced to retry with full context.',
							);
						}
						...
						tokens.seek(0);
						if (
							TestPerformance.REUSE_PARSER &&
							TestPerformance.sharedParsers[thread] != null
						) {
							parser.inputStream = tokens;
						} else {
							if (TestPerformance.USE_PARSER_INTERPRETER) {
						...
						if (!TestPerformance.ENABLE_PARSER_DFA) {
							parser.interpreter =
								new NonCachingParserATNSimulator(
									parser.atn,
									parser,
								);
							...
							parser.interpreter =
								new StatisticsParserATNSimulator(
									TestPerformance.sharedParserATNs[thread]!,
									parser,
								);
					...
					return new FileParseResult(
						...
						parseResult,
						tokens.size,
						TestPerformance.TIME_PARSE_ONLY ? parseStartTime : (
							startTime
						),
						lexer,
						parser,
					...
					if (
						!TestPerformance.REPORT_SYNTAX_ERRORS &&
	...
	constructor(
		...
		if (parser != null) {
			...
			if (interpreter instanceof StatisticsParserATNSimulator) {
				...
				this.parserComputedTransitions =
					interpreter.computedTransitions;
				this.parserFullContextTransitions =
					interpreter.fullContextTransitions;
			} else {
...
class StatisticsParserATNSimulator extends ParserATNSimulator {
	...
	constructor(atn: ATN, parser: Parser) {
		...
		this.fullContextTransitions = new Uint32Array(
			atn.decisionToState.length,
		);
		this.decision = -1;
	}
...
class DescriptiveErrorListener implements ParserErrorListener {
	public static INSTANCE: DescriptiveErrorListener =
	...
	public syntaxError<T extends Token>(
		...
		let sourceName: string =
			inputStream != null ? inputStream.sourceName : '';
...
class DescriptiveLexerErrorListener implements ANTLRErrorListener<number> {
	public static INSTANCE: DescriptiveLexerErrorListener =
		new DescriptiveLexerErrorListener();
...
class SummarizingDiagnosticErrorListener extends DiagnosticErrorListener {
	...
	public reportAmbiguity(
		...
		if (
			...
			let llPredictions: BitSet = this.getConflictingAlts(
				ambigAlts,
				configs,
			);
...
export class InputDescriptor {
	...
	public getInputStream(): CharStream {
		if (this.inputStream === undefined) {
			this.inputStream = this.bufferFromFileName(
				this.source,
				TestPerformance.ENCODING,
			);
		}

		return new JavaUnicodeInputStream(
			CodePointCharStream.fromBuffer(this.inputStream, this.source),
		);
	}

	private bufferFromFileName(
		source: string,
		encoding: string,
	): CodePointBuffer {
